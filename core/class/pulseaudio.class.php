...
class pulseaudio extends eqLogic
{
    ...
    public static function scanbluetoothpulseaudio()
    {
        ...
        $cmd  = 'expect ' . dirname(__FILE__) . '/../../3rdparty/bluetooth-scan.sh ' . $port;
    ...
    public static function pairbluetoothpulseaudio($macAddress)
    {
        ...
        $port = config::byKey('port', 'pulseaudio', 0);
        $cmd  = 'expect ' . dirname(__FILE__) . '/../../3rdparty/bluetooth-pair-trust.sh ' . $port . ' ' . $macAddress;
    ...
    public static function statuspulseaudio($serviceName)
    {
        ...
        $cmd = 'systemctl -l status pulseaudio-speaker-service-' . $serviceName . '.service';
    ...
    {
        log::remove('pulseaudio_soundtest');
        $cmd = 'sudo kill -9 `cat ' . dirname(__FILE__) . '/../../pulseaudio_soundtest.pid`';
    ...
    public function preSave()
    {
        ...
        if (!$this->getConfiguration('lastName') == '') {
            ...
            if ($this->getConfiguration('name') !== $this->getConfiguration('lastName')) {
                ...
                $cmd = '/bin/bash ' . dirname(__FILE__) . '/../../3rdparty/disable.sh ' . $this->getConfiguration('lastName');
    ...
    public function postSave()
    {
        ...
        if ($this->getConfiguration('created') == false) {
            $cmd = '/bin/bash ' . dirname(__FILE__) . '/../../3rdparty/create.sh ' . $this->getConfiguration('name') . ' ' . $port . ' ' . $this->getConfiguration('address');
        ...
        if ($this->getIsEnable()) {
            $cmd = 'sudo /etc/init.d/pulseaudio-speaker-service-' . $this->getConfiguration('name') . ' start ';
            ...
            $cmd = 'sudo /etc/init.d/pulseaudio-speaker-service-' . $this->getConfiguration('name') . ' stop ';
    ...
    public function preRemove()
    {
        ...
        $port = config::byKey('port', 'pulseaudio', 0);
        $cmd  = '/bin/bash ' . dirname(__FILE__) . '/../../3rdparty/delete.sh ' . $this->getConfiguration('name') . ' ' . $port . ' ' . $this->getConfiguration('address');
